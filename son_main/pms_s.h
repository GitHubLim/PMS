...
#define LOWH 0					//색
#define HIGHH 60
#define LOWS 35 //21			채도
#define HIGHS 255
#define LOWV 40 //31			명도
#define HIGHV 255
...
Mat houghLine(Mat srcFrame) {
	...
	Mat edgeFrame;		//결과 외곽선
	GaussianBlur(srcFrame, srcFrame, Size(9, 9), 1.0);
	Canny(srcFrame, edgeFrame, 100, 150);				//canny(그레이레벨 영상, 결과 외곽선, 낮은 경계값, 높은 경계값);
	...
	const unsigned int kThresHoldForLine = 80;					//line의 threshold
	...
	for (int i = 0; i < nLineVecSize; i++)
	{
		...
		line(dstFrame, ptA, ptB, Scalar(0, 0, 255), 1);			//line을 그리는 함수
	}
...
Mat preMasking(Mat srcFrame) {
	...
	erode(binaryFrame, binaryFrame, getStructuringElement(MORPH_ELLIPSE, Size(ERODE, ERODE)));
	dilate(binaryFrame, binaryFrame, getStructuringElement(MORPH_ELLIPSE, Size(DILATE, DILATE)));

	dilate(binaryFrame, binaryFrame, getStructuringElement(MORPH_ELLIPSE, Size(ERODE, ERODE)));
	erode(binaryFrame, binaryFrame, getStructuringElement(MORPH_ELLIPSE, Size(DILATE, DILATE)));
